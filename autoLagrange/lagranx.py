    ...
    try:
        ...
        for epoch in range(num_epochs + 1):
            ...
            for batch in range(num_batches + 1):
                train_state, train_metrics = train_step(train_state, batch_train, lr_func)
...
def generate_data(settings):
    ...
    # create time
    t_train = np.arange(N, dtype=np.float32)  # time steps 0 to N
...
if __name__ == "__main__":
    ...
    learning_rate_fn = lambda t: jnp.select([t < settings['batch_size'] * (settings['num_batches'] // 3),
                                             t < settings['batch_size'] * (2 * settings['num_batches'] // 3),
                                             t > settings['batch_size'] * (2 * settings['num_batches'] // 3)],
                                            [1e-3, 3e-4, 1e-4])
