...
class black_box_model(nn.Module):
    ...
    def __call__(self, x, num_dof=4, net_size=64 * 4, friction=False):
        ...
        x_kin_1 = self.layer_kin(full_state, features=64 * 4)
        x_kin_2 = self.layer_kin(x_kin_1, features=64 * 4)
        x_kin_3 = self.layer_kin(x_kin_2, features=64 * 4)
        x_kin = nn.Dense(features=dim)(x_kin_1 + x_kin_2 + x_kin_3)

        # build potential net
        x_pot_1 = self.layer_pot(full_state, features=net_size)
        # x_pot_2 = self.layer_pot(x_pot_1, features=net_size)
        # x_pot_3 = self.layer_pot(x_pot_2, features=net_size)
        x_pot = nn.Dense(features=1)(x_pot_1)

        # # build friction net
        # x_f_1 = self.layer(full_state, features=net_size)
        # x_f_2 = self.layer(x_f_1, features=net_size)
        # x_f = nn.Dense(features=num_dof)(x_f_1 + x_f_2) * int(friction)
    ...
    def layer_kin(self, x: jnp.array, features: int = 128):
        x = nn.Dense(features=features)(x)
        x = nn.activation.tanh(x)
        return x

    def layer_pot(self, x: jnp.array, features: int = 128):
        x = nn.Dense(features=features)(x)
        x = nn.activation.tanh(x)
        return x
...
class DeLaNN(nn.Module):
    ...
    def __call__(self, x, num_dof=4, net_size=64 * 4, friction=False):
        ...
        # build potential net
        x_pot_1 = self.layer(q, features=64 * 10)
