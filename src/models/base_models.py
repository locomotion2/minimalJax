...
class BaseModel(ABC):
    ...
    def simulate(self, params: dict):
        ...
        output = solve_ivp(eqs, t_span=(ts[0], ts[-1]), y0=x_0, method='RK23', args=(eqs_params,), rtol=5e-2, atol=1e-5)
        # Convert output back to a JAX array
        x_final = jnp.asarray(output.y[:, -1])

        return x_final

    def update_trajectories(self, params: dict = None):
        """Correctly appends current state to trajectories using jnp.concatenate."""
